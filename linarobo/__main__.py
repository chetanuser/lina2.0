...
import linarobo.modules.sql.users_sql as sql
from linarobo import (
    BOT_NAME,
    BOT_USERNAME,
    LOGGER,
    OWNER_ID,
...
from linarobo.modules import ALL_MODULES
from linarobo.modules.helper_funcs.chat_status import is_user_admin
from linarobo.modules.helper_funcs.misc import paginate_modules
...
PM_START_TEXT = """ 
Hey there! My name is ᄂIПΛ2.0 
 I'm here to help you manage your groups! 
 Click help button  to find out more about how to use me to my full potential.
 

Are you interested in helping my creator with his efforts to keep me in active development? If yes, then hit /donate
"""

buttons = [
    [
        InlineKeyboardButton(
            text="add me to ur chat",
...
for module_name in ALL_MODULES:
    imported_module = importlib.import_module("linarobo.modules." + module_name)
...
@run_async
def lina_about_callback(update: Update, context: CallbackContext):
    query = update.callback_query
    if query.data == "lina_":
        uptime = get_readable_time((time.time() - StartTime))
        query.message.edit_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
                            text="📍𝐎𝐖𝐍𝐄𝐑📍", url=f"https://t.me/thanosceo"
                        ),
                        InlineKeyboardButton(
                            text="🎄𝐑𝐄𝐏𝐎🎄", 
                    ...
                    [
                        InlineKeyboardButton(text="✯ вαϲк ✯", callback_data="lina_back"),
                    ],
                ]
            ),
        )
    elif query.data == "lina_back":
        first_name = update.effective_user.first_name 
        query.message.edit_text(
            PM_START_TEXT.format(escape_markdown(first_name), BOT_NAME),
            reply_markup=InlineKeyboardMarkup(buttons),
...
def donate(update: Update, context: CallbackContext):
    ...
    if chat.type == "private":
        ...
        if OWNER_ID != {OWNER_ID} and DONATION_LINK:
            update.effective_message.reply_text(
                f"» ᴛʜᴇ ᴅᴇᴠᴇʟᴏᴩᴇʀ ᴏғ {dispatcher.bot.first_name} sᴏᴜʀᴄᴇ ᴄᴏᴅᴇ ɪs [VIP BOY](https://t.me/thanosceo)"
...
""",
    ...
    about_callback_handler = CallbackQueryHandler(
        lina_about_callback, pattern=r"lina_"
    )
